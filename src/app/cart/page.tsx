...
const CartPage = () => {
  ...
  return (
    <div>
      ...
      <div className="px-24 py-14 flex w-full gap-8">
        <div className="w-3/4">
              ...
              <tbody>
                {CART_ITEMS.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td className="px-3 py-10">{item.product}</td>
                    <td className="px-3 py-10">{formatPriceARS(item.price)}</td>
                    <td className="px-3 py-10">
        ...
        <div className="w-1/4 bg-rose-300 pt-5 pb-16 px-10">
          ...
          <button className="mx-auto flex justify-center rounded-xl border py-[0.7rem] text-center w-3/4 border-black">
            Ir a Pagar
          </button>
        </div>
      </div>
